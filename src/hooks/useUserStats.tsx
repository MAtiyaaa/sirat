import { useEffect, useState } from 'react';
import { supabase } from '@/integrations/supabase/client';

export const useUserStats = (userId: string | undefined) => {
  const [stats, setStats] = useState({
    timesOpenedThisMonth: 0,
    timesOpenedThisYear: 0,
    daysOpenedThisYear: 0,
    surahsRead: 0,
    joinedDate: '',
  });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!userId) {
      setLoading(false);
      return;
    }

    const initializeStats = async () => {
      try {
        // Get profile to determine joined date
        const { data: profile } = await supabase
          .from('profiles')
          .select('created_at')
          .eq('user_id', userId)
          .maybeSingle();

        const joinedDate = profile?.created_at || '2025-10-01';

        // Get or create user stats
        const { data: existingStats, error: fetchError } = await supabase
          .from('user_stats')
          .select('*')
          .eq('user_id', userId)
          .maybeSingle();

        if (fetchError) throw fetchError;

        const today = new Date().toISOString().split('T')[0];
        const isNewDay = !existingStats || existingStats.last_opened_date !== today;
        const isNewMonth = !existingStats || 
          new Date(existingStats.last_opened_date).getMonth() !== new Date().getMonth();
        const isNewYear = !existingStats || 
          new Date(existingStats.last_opened_date).getFullYear() !== new Date().getFullYear();

        let updatedStats = existingStats;

        if (!existingStats) {
          // Create new stats record
          const { data: newStats, error: insertError } = await supabase
            .from('user_stats')
            .insert({
              user_id: userId,
              times_opened_this_month: 1,
              times_opened_this_year: 1,
              days_opened_this_year: 1,
              last_opened_date: today,
              surahs_read: 0,
            })
            .select()
            .single();

          if (insertError) throw insertError;
          updatedStats = newStats;
        } else if (isNewDay) {
          // Only update if it's a new day (unique daily tracking)
          const newTimesMonth = isNewMonth ? 1 : (existingStats.times_opened_this_month || 0) + 1;
          const newTimesYear = isNewYear ? 1 : (existingStats.times_opened_this_year || 0) + 1;
          const newDaysYear = isNewYear ? 1 : (existingStats.days_opened_this_year || 0) + 1;

          const { data: updated, error: updateError } = await supabase
            .from('user_stats')
            .update({
              times_opened_this_month: newTimesMonth,
              times_opened_this_year: newTimesYear,
              days_opened_this_year: newDaysYear,
              last_opened_date: today,
            })
            .eq('user_id', userId)
            .select()
            .single();

          if (updateError) throw updateError;
          updatedStats = updated;
        }

        // Count completed surahs from reading_progress
        const { data: progressData } = await supabase
          .from('reading_progress')
          .select('surah_number, ayah_number')
          .eq('user_id', userId);

        // Get surah ayah counts to determine completion
        let completedSurahs = 0;
        if (progressData) {
          const surahAyahCounts: Record<number, number> = {
            1: 7, 2: 286, 3: 200, 4: 176, 5: 120, 6: 165, 7: 206, 8: 75, 9: 129, 10: 109,
            11: 123, 12: 111, 13: 43, 14: 52, 15: 99, 16: 128, 17: 111, 18: 110, 19: 98, 20: 135,
            21: 112, 22: 78, 23: 118, 24: 64, 25: 77, 26: 227, 27: 93, 28: 88, 29: 69, 30: 60,
            31: 34, 32: 30, 33: 73, 34: 54, 35: 45, 36: 83, 37: 182, 38: 88, 39: 75, 40: 85,
            41: 54, 42: 53, 43: 89, 44: 59, 45: 37, 46: 35, 47: 38, 48: 29, 49: 18, 50: 45,
            51: 60, 52: 49, 53: 62, 54: 55, 55: 78, 56: 96, 57: 29, 58: 22, 59: 24, 60: 13,
            61: 14, 62: 11, 63: 11, 64: 18, 65: 12, 66: 12, 67: 30, 68: 52, 69: 52, 70: 44,
            71: 28, 72: 28, 73: 20, 74: 56, 75: 40, 76: 31, 77: 50, 78: 40, 79: 46, 80: 42,
            81: 29, 82: 19, 83: 36, 84: 25, 85: 22, 86: 17, 87: 19, 88: 26, 89: 30, 90: 20,
            91: 15, 92: 21, 93: 11, 94: 8, 95: 8, 96: 19, 97: 5, 98: 8, 99: 8, 100: 11,
            101: 11, 102: 8, 103: 3, 104: 9, 105: 5, 106: 4, 107: 7, 108: 3, 109: 6, 110: 3,
            111: 5, 112: 4, 113: 5, 114: 6
          };

          const surahProgress = new Map<number, number>();
          progressData.forEach(p => {
            surahProgress.set(p.surah_number, p.ayah_number);
          });

          surahProgress.forEach((ayahNum, surahNum) => {
            const totalAyahs = surahAyahCounts[surahNum];
            if (totalAyahs && ayahNum >= totalAyahs) {
              completedSurahs++;
            }
          });
        }

        if (updatedStats) {
          setStats({
            timesOpenedThisMonth: updatedStats.times_opened_this_month || 0,
            timesOpenedThisYear: updatedStats.times_opened_this_year || 0,
            daysOpenedThisYear: updatedStats.days_opened_this_year || 0,
            surahsRead: completedSurahs,
            joinedDate,
          });
        }
      } catch (error) {
        console.error('Error initializing user stats:', error);
      } finally {
        setLoading(false);
      }
    };

    initializeStats();
  }, [userId]);

  return { stats, loading };
};