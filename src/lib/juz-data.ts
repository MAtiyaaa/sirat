// Accurate Juz (part) boundary data for the Quran
// Based on the standard Medina Mushaf division
...
export const JUZ_BOUNDARIES: JuzBoundary[] = [
  ...
  { juz: 3, start: { surah: 2, ayah: 253 }, end: { surah: 3, ayah: 91 } },
  { juz: 4, start: { surah: 3, ayah: 92 }, end: { surah: 4, ayah: 23 } },
...
export function getJuzForAyah(surahNumber: number, ayahNumber: number): number {
  for (const boundary of JUZ_BOUNDARIES) {
    const afterStart =
      surahNumber > boundary.start.surah ||
      (surahNumber === boundary.start.surah && ayahNumber >= boundary.start.ayah);
    const beforeEnd =
      surahNumber < boundary.end.surah ||
      (surahNumber === boundary.end.surah && ayahNumber <= boundary.end.ayah);
    if (afterStart && beforeEnd) return boundary.juz;
  }
  return 1;
}

// Get Juz boundaries that fall WITHIN a specific surah (i.e., where a new Juz starts mid-surah)
export function getJuzBoundariesInSurah(
  surahNumber: number,
  _totalAyahs: number
): { ayah: number; juzEnding: number; juzStarting: number }[] {
  const markers: { ayah: number; juzEnding: number; juzStarting: number }[] = [];

  for (const boundary of JUZ_BOUNDARIES) {
    // A Juz boundary falls inside this surah if:
    // - The Juz starts at this surah AND not at ayah 1 (meaning the previous Juz ended here)
    // - OR the Juz starts at this surah at ayah 1 but it's not the first Juz of the surah
    if (boundary.start.surah === surahNumber && boundary.start.ayah > 1) {
      markers.push({
        ayah: boundary.start.ayah,
        juzEnding: boundary.juz - 1,
        juzStarting: boundary.juz,
      });
    }
  }

  return markers.sort((a, b) => a.ayah - b.ayah);
}

// Juz Arabic names (optional decorative names)
export const JUZ_NAMES_AR: Record<number, string> = {
  1: 'آلم', 2: 'سيقول', 3: 'تلك الرسل', 4: 'لن تنالوا',
  5: 'والمحصنات', 6: 'لا يحب الله', 7: 'وإذا سمعوا',
  8: 'ولو أننا', 9: 'قال الملأ', 10: 'واعلموا',
  11: 'يعتذرون', 12: 'وما من دابة', 13: 'وما أبرئ',
  14: 'ربما', 15: 'سبحان الذي', 16: 'قال ألم',
  17: 'اقترب للناس', 18: 'قد أفلح', 19: 'وقال الذين',
  20: 'أمّن خلق', 21: 'اتل ما أوحي', 22: 'ومن يقنت',
  23: 'وما لي', 24: 'فمن أظلم', 25: 'إليه يرد',
  26: 'حم', 27: 'قال فما خطبكم', 28: 'قد سمع الله',
  29: 'تبارك الذي', 30: 'عمّ',
};
