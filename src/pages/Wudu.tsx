...
const Wudu = () => {
  ...
  // === AUTO REQUEST PERMISSION WHEN QIBLA OPENS ===
  useEffect(() => {
    if (isQiblaOpen && !permissionGranted && !isCalibrating) {
      requestOrientationPermission();
    }
  }, [isQiblaOpen]);

  // === QIBLA ORIENTATION EFFECT ===
  useEffect(() => {
                  ...
                  {permissionGranted ? (
                    ...
                    <div className="glass-effect rounded-2xl p-4 border border-primary/20 bg-primary/5">
                      <div className="flex items-center justify-center">
                        <span className="text-sm text-muted-foreground">
                          {settings.language === "ar"
                            ? "البوصلة ستفعل تلقائياً"
                            : "Compass will activate automatically"}
                        </span>
                      </div>
                    </div>
                  )}
