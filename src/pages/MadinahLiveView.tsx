...
const MadinahLiveView = () => {
  const navigate = useNavigate();
  const { settings } = useSettings();

  useEffect(() => {
    // Redirect to external live stream
    window.location.href = "https://www.makkah.live/madinah-live";
  }, []);

  const content = {
    title: settings.language === 'ar' ? 'البث المباشر للمسجد النبوي' : "Prophet's Mosque Live View",
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 flex flex-col items-center justify-center">
      <div className="max-w-2xl mx-auto space-y-6 text-center">
        ...
        <Button 
          onClick={() => window.location.href = "https://www.makkah.live/madinah-live"}
