...
/* ------------------ Types ------------------ */
type CategoryKey =
  | "all" | "morning" | "evening" | "sleep" | "waking" | "eating" | "home" | "travel" | "protection"
  | "knowledge" | "forgiveness" | "parents" | "rain" | "istikhara" | "wudu" | "salah" | "guidance"
  | "health" | "anxiety" | "gratitude" | "rizq" | "repentance" | "patience" | "character" | "mercy"
  | "marriage" | "work" | "calamity" | "illness" | "visiting_sick" | "graveyard" | "meeting"
  | "debt" | "study" | "general";
...
const ALL_DUAS: Dua[] = [
  ...
  // Additional Authentic Duas
  {
    id: "yunus-dua",
    icon: Shield,
    categories: ["calamity", "forgiveness", "protection"],
    titleAr: "دعاء يونس",
    titleEn: "Yunus's Dua",
  ...
  },
  {
    id: "laylatul-qadr",
    icon: Moon,
    categories: ["evening", "forgiveness", "general"],
  ...
  {
    id: "comprehensive-dua",
    icon: Heart,
    categories: ["general", "gratitude", "guidance"],
    titleAr: "دعاء جامع",
    ...
    transliteration: "Allāhumma innī as'aluka minal-khayri kullihī ʿājilihī wa ājilih, mā ʿalimtu minhu wa mā lam aʿlam.",
    translation: "O Allah, I ask You for all that is good, in this world and the Hereafter, what I know of it and what I do not know.",
    reference: "Ibn Majah",
  },
  {
    id: "heart-guidance",
    icon: Compass,
    categories: ["guidance", "character"],
    titleAr: "دعاء هداية القلب",
    titleEn: "Guidance of the Heart",
  ...
  },
  {
    id: "illness-cure",
    icon: Hospital,
    categories: ["illness", "health"],
    titleAr: "دعاء الشفاء",
    titleEn: "For Cure from Illness",
    arabic: "أَذْهِبِ الْبَاسَ رَبَّ النَّاسِ، اشْفِ أَنْتَ الشَّافِي، لَا شِفَاءَ إِلَّا شِفَاؤُكَ",
    transliteration: "Adhhibil-ba'sa rabban-nās, ishfi antash-shāfī, lā shifā'a illā shifā'uk.",
    translation: "Remove the hardship, O Lord of mankind, heal, for You are the Healer. There is no healing except Your healing.",
    reference: "Bukhari, Muslim",
  },
  {
    id: "marriage-dua",
    icon: Heart,
    categories: ["marriage", "parents"],
    titleAr: "دعاء الزواج",
    titleEn: "Marriage Dua",
    arabic: "بَارَكَ اللَّهُ لَكَ، وَبَارَكَ عَلَيْكَ، وَجَمَعَ بَيْنَكُمَا فِي خَيْرٍ",
    transliteration: "Bārakallāhu lak, wa bāraka ʿalayk, wa jamaʿa baynakumā fī khayr.",
    translation: "May Allah bless you, shower His blessings upon you, and join you together in goodness.",
    reference: "Abu Dawud, Tirmidhi",
  },
  {
    id: "quran-17-24",
    icon: Users,
    categories: ["parents", "mercy"],
  ...
  },
  {
    id: "distress-removal",
    icon: Gauge,
    categories: ["anxiety", "calamity", "protection"],
    titleAr: "دعاء الكرب",
    titleEn: "Removing Distress",
    arabic: "اللَّهُمَّ رَحْمَتَكَ أَرْجُو فَلَا تَكِلْنِي إِلَى نَفْسِي طَرْفَةَ عَيْنٍ",
    transliteration: "Allāhumma raḥmataka arjū fa-lā takilnī ilā nafsī ṭarfata ʿayn.",
    translation: "O Allah, it is Your mercy that I hope for, so do not leave me in charge of my affairs even for a blink of an eye.",
    reference: "Abu Dawud",
  },
  {
    id: "seeking-jannah",
    icon: Heart,
    categories: ["general", "guidance"],
    titleAr: "طلب الجنة",
    titleEn: "Seeking Paradise",
    arabic: "اللَّهُمَّ إِنِّي أَسْأَلُكَ الْجَنَّةَ وَمَا قَرَّبَ إِلَيْهَا مِنْ قَوْلٍ أَوْ عَمَلٍ",
    transliteration: "Allāhumma innī as'alukal-jannata wa mā qarraba ilayhā min qawlin aw ʿamal.",
    translation: "O Allah, I ask You for Paradise and whatever brings one closer to it of speech or action.",
...
const Duas: React.FC = () => {
  ...
  const toggleBookmark = async (dua: Dua) => {
    if (!user) {
      toast.error(langIsAr ? "يجب تسجيل الدخول أولاً" : "Please sign in first");
      navigate("/auth");
      return;
    }
    const key = dua.titleEn;
    const isSaved = bookmarkedDuas.has(key);
    if (isSaved) {
      const { error } = await supabase
        ...
        .eq("user_id", user.id)
        .eq("dua_title", key);
      if (error) return toast.error(langIsAr ? "فشل الحذف" : "Failed to remove");
      setBookmarkedDuas((p) => {
        const n = new Set(p);
        n.delete(key);
        return n;
      });
      toast.success(langIsAr ? "تم الحذف" : "Bookmark removed");
    } else {
      ...
      if (error) return toast.error(langIsAr ? "فشل الحفظ" : "Failed to bookmark");
      setBookmarkedDuas((p) => new Set(p).add(key));
      toast.success(langIsAr ? "تم الحفظ" : "Bookmarked");
    }
  };

  /* ---------- Share (no copy button) ---------- */
  const buildShareUrl = (dua: Dua) => {
  ...
  const shareDua = async (dua: Dua) => {
    const url = buildShareUrl(dua);
    try {
      if (navigator.share) {
        await navigator.share({
          title: langIsAr ? dua.titleAr : dua.titleEn,
          text: langIsAr ? dua.arabic : dua.translation,
          url,
        });
      } else {
  ...
  const filtered = useMemo(() => {
    const byCat =
      category === "all" ? ALL_DUAS : ALL_DUAS.filter((d) => d.categories.includes(category));
