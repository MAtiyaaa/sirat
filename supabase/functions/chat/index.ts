...
serve(async (req) => {
  ...
  try {
    ...
    const { messages, userId, userFirstName, userFullName, isPrivateMode } = JSON.parse(body);
...
- The user wants privacy, so be respectful of that choice`;
      ...
      const formatCompletedSurahs = (surahs: any[]) => {
        if (!surahs || surahs.length === 0) return "None yet";
        return surahs.map((s: any) => `${surahNames[s.surah_number] || `Surah ${s.surah_number}`} (${s.surah_number})`).join("\n");
      };

      userContext = `
User Information:
- Full Name: ${userFullName || "Unknown"}
- First Name: ${userFirstName || "friend"}
${progress ? `- Currently reading: Surah ${surahNames[progress.surah_number] || progress.surah_number} (${progress.surah_number}), Ayah ${progress.ayah_number}` : ""}
${lastViewed ? `- Last viewed: Surah ${surahNames[lastViewed.surah_number] || lastViewed.surah_number} (${lastViewed.surah_number})` : ""}
${bookmarksCount ? `- Total bookmarks: ${bookmarksCount}` : ""}
${completedSurahs && completedSurahs.length > 0 ? `- Completed surahs:\n${formatCompletedSurahs(completedSurahs)}` : ""}

CRITICAL FORMATTING RULES:
1. When addressing the user, ALWAYS use their first name only (e.g., "Hi ${userFirstName}")
2. NEVER say "Surah 1" or "Surah 2" - ALWAYS use full names like "Surah Al-Fatiha" or "Surah Al-Baqarah"
3. For ayah references, "Ayah 1" format is acceptable
4. When listing completed surahs, format each on a new line as shown above: "Surah Name (Number)"
5. Do not mention or reference the user's email address`;
    }
